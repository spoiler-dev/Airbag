...
export default {
  ...
  mounted() {
        ...
        .then(res => {
          ...
          if (res.data[0].partSize) {
            ...
            for (let i = 0; i < res.data[0].equipInfo.length; i++) {
              ...
              if (res.data[0].equipInfo[i].eqType == '02') {
                type = 'temperature'
              } else if (res.data[0].equipInfo[i].eqType == '03') {
                type = 'smoke'
              } else if (res.data[0].equipInfo[i].eqType == '25' || '29') {
                type = 'control'
              }
              name = res.data[0].equipInfo[i].eqName;
    ...
    panelBtn.addEventListener('click', function () {
      ...
      let axis;
      // 根据输入的x、y坐标改变当前模块的位置
      let xChange = (axisX.value - (_this.ontology.offsetWidth / 2) + 530) + 'px';
      let yChange = (axisY.value - (_this.ontology.offsetHeight / 2) + 250) + 'px';
    ...
    // 获取坐标数据
    function data(machinePositionArr, airPositionArr, smokePositionArr, temperaturePositionArr, controllerPositionArr,
      fontPositionArr, describArr, machineNameArr, machineNamePositionArr, airNameArr, airNamePositionArr) {
      zone = document.getElementById('love-house');
      // x轴距离浏览器窗口顶部的位置
      let xTop = zone.getBoundingClientRect().top + zone.offsetHeight / 2;
      // y轴距离浏览器窗口左部的位置
    ...
    let three = document.getElementById('three-dimensional');
    // 平面视图切换
    ...
    tab2Btn.addEventListener('click', function () {
      ...
      data(machinePositionArr, airPositionArr, smokePositionArr, temperaturePositionArr, controllerPositionArr,
        fontPositionArr, describArr, machineNameArr, machineNamePositionArr, airNameArr, airNamePositionArr);
      threeRoom(gridWidth, gridLength, machinePositionArr, airPositionArr, smokePositionArr,
        temperaturePositionArr, controllerPositionArr, describArr);
      // 数据清空
      machinePositionArr = [];
      airPositionArr = [];
      smokePositionArr = [];
    ...
    // 3D场景构造
    function workshop() {
      three.width = _this.gridWidth
      three.height = _this.gridLength
      // 建造场景
      function initScene() {
      ...
      function initModel() {
        ...
        function room() {
          // 墙
          let wall1 = new THREE.Object3D()

          let wallBottomGeometry = new THREE.BoxGeometry(20, 90, gridLength)
          let wallBottom = utils.makeMesh('phong', wallBottomGeometry, 0xC6E2FF)
          wallBottom.position.set(0, 45, 0)
          wall1.add(wallBottom)
          let holeGeometry = new THREE.BoxGeometry(4, 200, gridLength - 10)
          let hole = utils.makeMesh('glass', holeGeometry, 0x00BFFF, 0.6)
          hole.position.set(0, 145, 0)
          wall1.add(hole)
          let wallTopGeometry = new THREE.BoxGeometry(20, 30, gridLength)
          let wallTop = utils.makeMesh('phong', wallTopGeometry, 0xC6E2FF)
          wallTop.position.set(0, 240, 0)
          wall1.add(wallTop)
          wall1.position.set((-gridWidth / 2) - 10, -5, 0)
          scene.add(wall1)

          let wall2 = wall1.clone()
          wall2.position.set((gridWidth / 2) + 10, -5, 0)
          scene.add(wall2)
        }
        // 机柜
        function machine() {
  ...
  methods: {
    ...
    listener() {
      ...
      window.addEventListener('resize', () => {
        if (document.documentElement.clientWidth || document.body.clientWidth < 750)
          _this.$message({
